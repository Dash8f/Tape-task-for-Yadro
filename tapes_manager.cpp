...
Tape TapesManager::create_new_tape(std::string tape_name, size_t len_of_tape)
{
    Tape tape;

    tape.tape_size = len_of_tape;

    tape.tape_name = tape_name;

    tape.tape_file.open(tape_name, std::ios::out | std::ios::in | std::ios::trunc);

    tape.tape_file << '<';
    for(int i = 0; i < len_of_tape;  ++i)
    {
        tape.tape_file <<  BIN_ZERO << ' ';
    }

    tape.tape_file.seekp(-1, std::ios_base::cur);
    tape.tape_file << '>';

    tape.tape_file.seekp(0, std::ios_base::beg);
...
{ 
    Tape merged_tape = create_new_tape(new_tape_name, (tape_1->tape_size + tape_2->tape_size));
...
Tape TapesManager::merge_sort_tape(Tape* tape_to_sort)
{
    ...
    for(size_t files_cnt = 0; files_cnt < amount_of_files; ++files_cnt)
    {
        ...
        Tape tmp_tape = create_new_tape("/tmp/" + std::to_string(files_cnt) + ".txt", M);
    ...
    if(N % M != 0)
    {
        ...
        Tape tmp_tape = create_new_tape("/tmp/" + std::to_string(amount_of_files - 1) + ".txt", N % M);
    ...
    return create_new_tape("test", 10);
}